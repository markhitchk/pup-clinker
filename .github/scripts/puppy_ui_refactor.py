...
def rep(old, new, count=1):
    ...
    found = s.count(old)
    if found < count:
        raise SystemExit(f'Expected at least {count} match(es), found {found}: {old[:120]!r}')
...
''', '''        Row(
        ...
        Surface(
            ...
            Box(Modifier.fillMaxSize()) {
''')
rep('.size(294.dp)', '.size(258.dp)')
rep('V6PuppyPortrait(state.puppyStyle, 270.dp, state.accessory)', 'V6PuppyPortrait(state.puppyStyle, 238.dp, state.accessory)')

rep('''        Spacer(Modifier.height(10.dp))
...
'''
s = s.replace(marker, prestige_screen + marker, 1)
...
settings_at = s.find('private fun V6Settings')
start = s.find('    if (prestigeConfirm) {', settings_at)
end_marker = '\n\n    if (resetConfirm) {'
end = s.find(end_marker, start)
if start == -1 or end == -1:
    raise SystemExit('Could not remove prestige dialog from Settings')
s = s[:start] + '    if (resetConfirm) {' + s[end + len(end_marker):]

old_wallet = '''@Composable
...
rep(old_wallet, new_wallet)

path.write_text(s)

gradle = Path('app/build.gradle.kts')
g = gradle.read_text()
if 'versionCode = 11' not in g or 'versionName = "1.6.1"' not in g:
    raise SystemExit('Expected v1.6.1/versionCode 11 before UI patch')
g = g.replace('versionCode = 11', 'versionCode = 12', 1)
g = g.replace('versionName = "1.6.1"', 'versionName = "1.6.2"', 1)
gradle.write_text(g)
